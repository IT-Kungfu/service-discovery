...
type Hook struct {
	...
	levels                   map[logrus.Level]struct{}
	TimeFormat               string
...
func (h *Hook) levelExists(level logrus.Level) bool {
	if h.levels == nil {
		return true
	}
	if len(h.levels) == 0 {
...
func (h *Hook) SetLevels(levels []logrus.Level) {
	h.levels = make(map[logrus.Level]struct{}, len(levels))
